...
class CMYKRGConverterSimple:
    ...
    def cambiar_resolucion(self, resolucion):
        """Cambiar modelo según resolución seleccionada - VERSIÓN MEJORADA"""
        if resolucion == "600":
            if self.modelo_600 is not None:
                ...
                self.modelo_actual = None
                if st.session_state.tipo_usuario == "tecnico":
                    st.error(f"❌ No hay modelo disponible para 600 DPI")
                    
        elif resolucion == "1200":
            if self.modelo_1200 is not None:
                ...
                self.modelo_actual = None
                if st.session_state.tipo_usuario == "tecnico":
                    st.error(f"❌ No hay modelo disponible para 1200 DPI")
    
    def verificar_estado_modelos(self):
        """Verificar estado actual de los modelos"""
        estado = {
            'modelo_600': self.modelo_600 is not None,
            'modelo_1200': self.modelo_1200 is not None,
            'modelo_actual': self.modelo_actual is not None,
            'resolucion_actual': st.session_state.get("resolucion_select", "No seleccionada")
        }
        return estado
    ...
    def cargar_modelo_manual(self, resolucion, uploaded_model):
        """Cargar un modelo manualmente - SOLO TÉCNICOS - VERSIÓN MEJORADA"""
        try:
            ...
            if resolucion == "600":
                ...
                # ACTUALIZAR MODELO ACTUAL SI ESTÁ EN USO
                if st.session_state.get("resolucion_select") == "600":
                    self.modelo_actual = self.modelo_600
                    self.scaler_actual = self.scaler_600
                    st.success("🔧 Modelo 600 DPI activado inmediatamente")
                ...
                # ACTUALIZAR MODELO ACTUAL SI ESTÁ EN USO
                if st.session_state.get("resolucion_select") == "1200":
                    self.modelo_actual = self.modelo_1200
                    self.scaler_actual = self.scaler_1200
                    st.success("🔧 Modelo 1200 DPI activado inmediatamente")
        ...
        if st.session_state.tipo_usuario == "tecnico":
            ...
            with col_tec2:
                ...
                if modelo_file:
                    ...
                    try:
                        ...
                        with open("temp_check.pkl", "wb") as f:
                            f.write(modelo_file.getbuffer())
                        
                        model_data = joblib.load("temp_check.pkl")
                        required_keys = ['model', 'scaler']
                        
                        if all(key in model_data for key in required_keys):
                            st.success("✅ Estructura del modelo válida")
                            if st.button("📥 Cargar Modelo", width='stretch'):
                                # Resetear el file upload para poder leerlo de nuevo
                                modelo_file.seek(0)
                        ...
                        os.remove("temp_check.pkl")
                        
                    except Exception as e:
                        st.error(f"❌ Error verificando modelo: {str(e)}")
                        try:
                            os.remove("temp_check.pkl")
                        except:
                            pass
            ...
            with col_tec3:
                if st.button("📊 Ver Info del Sistema", width='stretch'):
                    estado = self.verificar_estado_modelos()
                    st.write("**Estado de Modelos:**")
                    st.write(f"- Modelo 600 DPI: {'✅ Cargado' if estado['modelo_600'] else '❌ No cargado'}")
                    st.write(f"- Modelo 1200 DPI: {'✅ Cargado' if estado['modelo_1200'] else '❌ No cargado'}")
                    st.write(f"- Modelo Actual: {'✅ Activo' if estado['modelo_actual'] else '❌ Inactivo'}")
                    st.write(f"- Resolución Seleccionada: {estado['resolucion_actual']}")
