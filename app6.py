...
class StreamlitAnalytics:
    def __init__(self):
        self.webhook_url = st.secrets.get("https://script.google.com/macros/s/AKfycbzSZ5TnNnA4W2JUpOUY6NG7w9nOO3QfHtJd04lNX3gJzeaLfP_JEJ20I7aQG3AXvims/exec", "")
...
# =============================================================================
# INTERFAZ DE CONVERSIÓN DE MODELOS - SOLO PARA TÉCNICOS
# =============================================================================

class ConvertidorModelos:
    def __init__(self, app):
        self.app = app
    
    def mostrar_interfaz(self):
        """Interfaz para convertir y reparar modelos"""
        st.header("🛠️ Convertidor de Modelos .pkl")
        st.warning("⚠️ **SOLO PARA TÉCNICOS** - Esta herramienta puede modificar archivos del sistema")
        ...
        with tab1:
            self._analizar_modelos()
        
        with tab2:
            self._convertir_modelos()
        
        with tab3:
            self._entrenar_nuevos_modelos()
        
        with tab4:
            self._reparacion_emergencia()
    
    def _analizar_modelos(self):
        """Analizar modelos existentes"""
        st.subheader("🔍 Análisis de Modelos Existentes")
        
        modelos_dir = self.app.obtener_directorio_modelos()
        ...
        st.success(f"📁 Encontrados {len(pkl_files)} archivos .pkl")
        
        for file_path in pkl_files:
            with st.expander(f"📄 {os.path.basename(file_path)}", expanded=False):
                self._analizar_modelo_individual(file_path)
    
    def _analizar_modelo_individual(self, file_path):
        """Analizar un modelo individual"""
        ...
        with col1:
            ...
            st.write(f"**Lectura:** {'✅' if readable else '❌'}")
            st.write(f"**Escritura:** {'✅' if writable else '❌'}")
        ...
        with col2:
            ...
            try:
                ...
                try:
                    ...
                    st.success("✅ **joblib.load** - COMPATIBLE")
                    self._mostrar_info_modelo(model_data)
                    return
                except Exception as e1:
                    st.error(f"❌ **joblib.load** - {str(e1)[:100]}...")
                
                # Método 2: pickle estándar
                try:
                    import pickle
                    with open(file_path, 'rb') as f:
                        model_data = pickle.load(f)
                    st.success("✅ **pickle.load** - COMPATIBLE")
                    self._mostrar_info_modelo(model_data)
                    return
                except Exception as e2:
                    st.error(f"❌ **pickle.load** - {str(e2)[:100]}...")
                
                # Método 3: pickle con encoding alternativo
                try:
                    import pickle
                    with open(file_path, 'rb') as f:
                        model_data = pickle.load(f, encoding='latin1')
                    st.success("✅ **pickle.load(latin1)** - COMPATIBLE")
                    self._mostrar_info_modelo(model_data)
                    return
                except Exception as e3:
                    st.error(f"❌ **pickle.load(latin1)** - {str(e3)[:100]}...")
                
                st.error("🚨 **ARCHIVO INCOMPATIBLE** - No se pudo cargar con ningún método")
                
            except Exception as e:
                st.error(f"❌ Error en análisis: {e}")
    
    def _mostrar_info_modelo(self, model_data):
        """Mostrar información del modelo cargado"""
        if 'model' in model_data and 'scaler' in model_data:
            modelo = model_data['model']
            scaler = model_data['scaler']
            
            st.write("**Estructura:** ✅ Modelo + Scaler")
            st.write(f"**Tipo modelo:** {type(modelo).__name__}")
            
            if hasattr(modelo, 'n_features_in_'):
                st.write(f"**Características:** {modelo.n_features_in_}")
            if hasattr(modelo, 'n_estimators'):
                st.write(f"**Estimadores:** {modelo.n_estimators}")
            
            st.write(f"**Tipo scaler:** {type(scaler).__name__}")
        else:
            st.error("❌ Estructura inválida - No tiene 'model' y 'scaler'")
    
    def _convertir_modelos(self):
        """Interfaz para convertir modelos"""
        st.subheader("🔄 Conversión de Formatos .pkl")
        
        # Subir modelo para conversión
        modelo_upload = st.file_uploader(
            "📤 Subir modelo .pkl para convertir",
            type=['pkl'],
            key="convert_upload"
        )
        ...
        if modelo_upload:
            ...
            with col1:
                ...
                st.write(f"- Nombre: {modelo_upload.name}")
                st.write(f"- Tamaño: {modelo_upload.size:,} bytes")
                
                # Opciones de conversión
                formato_salida = st.selectbox(
                    "Formato de salida:",
                    ["pickle (protocol=4) - MÁXIMA COMPATIBILIDAD",
                     "joblib - COMPRESIÓN OPTIMIZADA",
                     "pickle (protocol=5) - MÁXIMA EFICIENCIA"]
                )
                
                nombre_salida = st.text_input(
                    "Nombre archivo de salida:",
                    value=f"compatible_{modelo_upload.name}"
                )
            
            with col2:
                if st.button("🔄 CONVERTIR MODELO", type="primary", use_container_width=True):
                    with st.spinner("Convirtiendo modelo..."):
                        resultado = self._convertir_modelo_uploaded(
                            modelo_upload, nombre_salida, formato_salida
                        )
                        
                        if resultado:
                            st.success("✅ Conversión exitosa!")
                            st.info(f"**Archivo guardado como:** {nombre_salida}")
                            
                            # Ofrecer descarga
                            with open(nombre_salida, "rb") as f:
                                st.download_button(
                                    "📥 Descargar modelo convertido",
                                    f,
    ...
    def _convertir_modelo_uploaded(self, uploaded_file, output_name, formato):
        """Convertir modelo subido"""
        try:
            ...
            # Cargar modelo (con todos los métodos)
            model_data = self._cargar_modelo_con_todos_metodos(temp_path)
            if not model_data:
                st.error("❌ No se pudo cargar el modelo con ningún método")
                return False
            
            # Guardar en formato seleccionado
            if "pickle (protocol=4)" in formato:
                import pickle
                with open(output_name, 'wb') as f:
                    pickle.dump(model_data, f, protocol=4)
            
            elif "joblib" in formato:
                joblib.dump(model_data, output_name, compress=3)
            
            elif "pickle (protocol=5)" in formato:
                import pickle
                with open(output_name, 'wb') as f:
    ...
    def _cargar_modelo_con_todos_metodos(self, file_path):
        """Intentar cargar modelo con todos los métodos posibles"""
        métodos = [
            # Método joblib
            lambda: joblib.load(file_path),
            
            # Métodos pickle con diferentes encodings
            lambda: self._cargar_pickle(file_path, 'utf-8'),
            lambda: self._cargar_pickle(file_path, 'latin1'),
            lambda: self._cargar_pickle(file_path, 'bytes'),
            lambda: self._cargar_pickle_ignore_errors(file_path)
        ]
    ...
    def _cargar_pickle(self, file_path, encoding):
        """Cargar con pickle y encoding específico"""
        import pickle
        with open(file_path, 'rb') as f:
            return pickle.load(f, encoding=encoding)
    
    def _cargar_pickle_ignore_errors(self, file_path):
        """Cargar con pickle ignorando errores"""
        import pickle
        with open(file_path, 'rb') as f:
            return pickle.load(f, errors='ignore')
    
    def _entrenar_nuevos_modelos(self):
        """Interfaz para entrenar modelos nuevos"""
        st.subheader("📊 Entrenar Modelos Nuevos")
        st.info("💡 Crea modelos compatibles desde cero")
        ...
        with col1:
            ...
            nombre_modelo = st.text_input(
                "Nombre del modelo:",
                value=f"modelo_{tipo_modelo.lower()}_dpi"
            )
        
        with col2:
            st.write("**Datos de Entrenamiento:**")
            st.warning("⚠️ Esta función requiere datos de entrenamiento")
            
            # Upload de datos de entrenamiento
            datos_train = st.file_uploader(
                "📊 Subir datos de entrenamiento (CSV)",
                type=['csv'],
                key="train_data"
            )
            
            if datos_train:
                st.success(f"✅ Datos cargados: {datos_train.name}")
                
                if st.button("🎯 ENTRENAR NUEVO MODELO", type="primary", use_container_width=True):
                    with st.spinner("Entrenando modelo..."):
                        # Aquí iría la lógica de entrenamiento
                        st.info("🔧 Función de entrenamiento en desarrollo...")
                        st.info("💡 Por ahora, usa la conversión de modelos existentes")
    
    def _reparacion_emergencia(self):
        """Reparación de emergencia para modelos corruptos"""
        st.subheader("🚨 Reparación de Emergencia")
        st.error("⚠️ **USO AVANZADO** - Solo si otros métodos fallan")
        
        st.write("""
        **Esta herramienta intenta:**
        - Recuperar modelos parcialmente corruptos
        - Reconstruir estructura de datos
        - Forzar compatibilidad extrema
        """)
        
        modelo_emergencia = st.file_uploader(
            "🚨 Subir modelo para reparación de emergencia",
            type=['pkl'],
            key="emergency_upload"
        )
        
        if modelo_emergencia:
            if st.button("🛠️ EJECUTAR REPARACIÓN DE EMERGENCIA", 
                        type="secondary", 
                        use_container_width=True):
                
                with st.spinner("Ejecutando reparación avanzada..."):
                    resultado = self._reparacion_avanzada(modelo_emergencia)
                    
                    if resultado:
                        st.success("✅ Reparación exitosa!")
                        
                        # Descargar modelo reparado
                        with open("modelo_reparado.pkl", "rb") as f:
                            st.download_button(
                                "📥 Descargar modelo reparado",
                                f,
                                file_name="modelo_reparado_compatible.pkl",
                                mime="application/octet-stream"
                            )
                    else:
                        st.error("❌ Reparación fallida - Modelo muy dañado")
    
    def _reparacion_avanzada(self, uploaded_file):
        """Reparación avanzada de modelos"""
        try:
            # Esta es una implementación básica - se puede expandir
            temp_path = f"emergency_{uploaded_file.name}"
            with open(temp_path, "wb") as f:
                f.write(uploaded_file.getbuffer())
            
            # Intentar reconstrucción byte a byte
            with open(temp_path, 'rb') as f:
                raw_data = f.read()
            
            # Buscar patrones de datos conocidos
            # (Implementación simplificada)
            if b'RandomForest' in raw_data or b'scaler' in raw_data:
                # Intentar reconstrucción manual
                st.info("🔍 Patrones de modelo detectados - intentando reconstrucción...")
                
                # Aquí iría lógica más avanzada de reconstrucción
                # Por ahora, solo conversión básica
                model_data = self._cargar_modelo_con_todos_metodos(temp_path)
                
                if model_data:
                    # Guardar reparado
                    import pickle
                    with open("modelo_reparado.pkl", 'wb') as f:
                        pickle.dump(model_data, f, protocol=4)
...
class CMYKRGConverterSimple:
    def __init__(self):
        ...
        self.convertidor = ConvertidorModelos(self)  # <- Agrega el convertidor
        self.cargar_modelos()
    ...
    def cargar_modelos(self):
        ...
        try:
            if st.session_state.tipo_usuario == "tecnico":
                st.info("🔍 BUSCANDO Y VERIFICANDO MODELOS...")
    ...
    def mostrar_interfaz_principal(self):
        ...
        if st.session_state.tipo_usuario == "tecnico":
            tab1, tab2, tab3, tab4 = st.tabs([
                "⚙️ Configuración y Cálculo", 
                "📊 Resultados", 
                "📋 Historial",
                "🛠️ Convertidor Modelos"
            ])
        else:
        ...
        if st.session_state.tipo_usuario == "tecnico":
            ...
            with tab4:
                self.convertidor.mostrar_interfaz()
